...
until pg_isready -U "$DB_USER" -p "$DB_PORT"; do
...
DB_EXISTS=$(psql -U "$DB_USER" -p "$DB_PORT" -tAc "SELECT 1 FROM pg_database WHERE datname = '$DB_NAME'")
if [[ "$DB_EXISTS" != "1" ]]; then
...
DB_EMPTY=$(psql -U "$DB_USER" -p "$DB_PORT" -d "$DB_NAME" -tAc "SELECT COUNT(*) = 0 FROM pg_tables WHERE schemaname = 'public';")
...
if [[ "$DB_EMPTY" == "t" ]]; then
  ...
  if [[ -f /backup/backup.sql ]]; then
    ...
    if psql -U "$DB_USER" -p "$DB_PORT" -d "$DB_NAME" < /backup/backup.sql; then
  ...
  psql -U "$DB_USER" -p "$DB_PORT" -d "$DB_NAME" -c "\dt"
fi
